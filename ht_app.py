...
from crud import HabitDAO,CategoryDAO,HabitLogDAO
from logic import HabitService, Utils
...
with tab1:
    ...
    daystreak = 0 #Stat.streak(daystreak) #need code migration to logic
    ...
    habit_list_all = None #.show_habit()
    ...
    with db_conn() as conn:
        ...
        for habit_id, name, done in habit_list:
            ...
            if new_done != bool(done):
                with db_conn() as conn:
                    ...
                    habit_service = HabitService(habit_dao)
                    if new_done:
                        habit_service.update_done(habit_id, done = 1)
                        st.success("Done")
                        time.sleep(0.7)
                        st.rerun()
                    else:
                        habit_service.update_done(habit_id, done = 0)
                        st.error("Undone")
                        time.sleep(0.7)
                        st.rerun()
        ...
        progress_bar = 0#Stat.progress_bar()
        st.divider()
    ...
    with tab2:
        ...
        with db_conn() as conn:
            ...
            if isinstance(habit_by_day, dict) and habit_by_day.get("success", False):
                st.warning(habit_by_day["error"])
            else:
                st.dataframe(habit_by_day, width='stretch', hide_index = True)
    ...
    with tab3:
        ...
        if cat_select == "Delete category":
            ...
            count = CategoryDAO.delete_category_validation(self,category_id)
